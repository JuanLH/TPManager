...
public class Utilities {
    ...
    //(d1.getYear()+1900)+"/"+(d1.getMonth()+1)+"/"+d1.getDate()
    
    public static float getGanancia(Date d1 ,Date d2){
        float ganancia=0.0f;
        DB dbase = Utilities.getConection();
        ...
        float ganancia_pago_pendiente=0,ganancia_pago_extras=0;//Variables que valen la ganancia de los pagos
        ...
        for(int x=0 ; x<lista_tipo_prestamo.size();x++){
            ...
            try{
                if(forma_prestamo_id != 2){
                    ...
                    rs.next();
                    /**/
                    float ganancia_pendiente = rs.getFloat(1);
                    ...
                    ganancia_pago_extras=rs.getFloat(1);  
                    rs.close();
                }
                else
                {
                    ...
                    rs.next();
                    /**/
                    ganancia_pago_pendiente = rs.getFloat(1);
                    rs = dbase.execSelect(query_extras);
                    rs.next();
            ...
            float ganancia_tp=0f;//variable que acumula la ganancia de un tipo de prestamo
    ...
    public static float getYMGanancia(Date d1 ,Date d2){
        float ganancia=0.0f;
        DB dbase = Utilities.getConection();
        ArrayList<Pago> lista_pagos = new ArrayList<Pago>();
        //array que devuelve la lista de pagos de un periodo de tiempo
    ...
    public static float getJXGanancia(Date d1 ,Date d2){
        float ganancia=0.0f;
        DB dbase = Utilities.getConection();
        ArrayList<Pago> lista_pagos = new ArrayList<Pago>();
        //array que devuelve la lista de pagos de un periodo de tiempo
        
        String query_pagos = "select id,id_tipo_pago,id_prestamo,fecha from pago where fecha >= '"+(d1.getYear()+1900)+"/"+(d1.getMonth()+1)+"/"+d1.getDate()+"' and fecha <= '"+(d2.getYear()+1900)+"/"+(d2.getMonth()+1)+"/"+d2.getDate()+"'";
        System.out.println(query_pagos);
        try{
            ResultSet rs = dbase.execSelect(query_pagos);
            while(rs.next()){
    ...
    public static float getGastos(Date d1 ,Date d2) throws SQLException{
        float gastos = 0.0f;
        Gastos g = new Gastos();
        ArrayList<Gastos> list = g.get_gastos(d1, d2);
        for(int x = 0; x<list.size();x++){
            float monto = list.get(x).getMonto();
            gastos = gastos + monto;
        }
    ...
    public static float getYMGastos(Date d1 ,Date d2) throws SQLException{
        float gastos = 0.0f;
        Gastos g = new Gastos();
        ArrayList<Gastos> list = g.get_YMGastos(d1, d2);
        for(int x = 0; x<list.size();x++){
            float monto = list.get(x).getMonto();
            gastos = gastos + monto;
        }
